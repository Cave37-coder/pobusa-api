...
# =============================================================================
# CATALOG — added to support Magic, One Piece, Dragon Ball Super (both
# variants), Digimon, Star Wars Unlimited, Gundam, Riftbound, Lorcana, plus
# a shared accessory catalog. Pokemon is DELIBERATELY NOT included here --
# Pokemon buy-ins keep working exactly as they do today, via live calls to
# pokemart-api's card_lookup/card_search. This catalog exists because
# pokemart-api only ever had Pokemon data -- there's nothing to call for
# any other game, so PoBuSA becomes its own source of truth for everything
# except Pokemon, synced directly from TCGCSV.
#
# Three tables, deliberately separated:
#   Game            -- which TCGs are supported, active/inactive per game
#   CatalogProduct  -- the searchable reference catalog (cards, sealed,
#                      accessories). This is NOT the bought-in inventory
#                      ledger -- CardStockLine/SealedStockItem still fill
#                      that role, snapshotting FROM this catalog at buy-in
#                      time, same relationship as pokemart-api already has.
#   TCGCSVSource    -- internal-only linkage back to TCGCSV's IDs. Never
#                      referenced by any serializer or Client-facing view.
#                      Only the sync command and Mike's own superuser admin
#                      ever touch this table. This is what "keep TCGCSV
#                      private" actually means in code: not an obfuscated
#                      field, a genuinely separate table with no API path
#                      to it at all.
# =============================================================================

class Game(models.Model):
    """One of PoBuSA's supported TCGs. A table, not a hardcoded choices list
    -- new games get added/activated without a migration."""
    code = models.SlugField(max_length=20, unique=True)  # e.g. "pokemon", "magic", "one-piece"
    name = models.CharField(max_length=100)  # e.g. "Pokémon", "Magic: The Gathering"

    # Internal only -- which TCGCSV category this game maps to for syncing.
    # Never exposed through any Client-facing serializer.
    tcgcsv_category_id = models.PositiveIntegerField(unique=True)

    is_active = models.BooleanField(default=True)  # lets you stage a game before going live with it
...
class CatalogProduct(models.Model):
    """The searchable reference catalog every Client's buy-in and sell
    screens search against -- cards, sealed product, and accessories,
    across every supported game (except Pokemon -- see module note above).
    """
    ...
    # PoBuSA's own SKU, game-prefixed, e.g. "mtg-xln-042-holo", "op-op01-012".
    # Generated by the sync command, never TCGCSV's own productId.
    ...
    # Null for accessories -- accessories are a shared, game-agnostic bucket,
    # not split per game. Required for single/sealed.
    game = models.ForeignKey(Game, on_delete=models.PROTECT, null=True, blank=True, related_name="products")
    ...
    # calculated and stored per-line on CardStockLine/SealedStockItem, this
    # is just the catalog's current snapshot.
    market_price = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True)

    # So Clients can hotlink or download a local copy -- see product
    # requirement: "if customers want them, they can be downloaded as
...
class TCGCSVSource(models.Model):
    """Internal-only. Maps a CatalogProduct back to its TCGCSV origin so
    pricing can be re-synced daily without ever exposing TCGCSV's IDs
